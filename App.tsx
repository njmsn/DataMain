...
import { MapArea } from './components/realTime/MapArea';
import { InspectionPanel } from './components/realTime/InspectionPanel';
...
const App: React.FC = () => {
  ...
  const closeAll = () => {
    ...
    setActiveTab(NavigationTab.RealTime);
    setContextMenu({ ...contextMenu, visible: false });
  };
      ...
      <div className={`flex-1 flex-col flex min-w-0 transition-all duration-300 gap-1 overflow-hidden ${isFullscreen ? 'p-0' : 'py-1 pr-2.5 pl-2.5'}`}>
        ...
        <div className={`flex-1 flex flex-col overflow-hidden relative transition-all duration-300 ${
          ...
          <div className={`flex-1 flex overflow-hidden transition-all duration-300 px-0 ${
            activeTab === NavigationTab.RealTime && !isFullscreen 
              ? 'py-[0.625rem] gap-0 bg-[#f1f3f6]' 
            ...
            <main className={`flex-1 relative overflow-hidden transition-all duration-300 ${
              ...
              {activeTab === NavigationTab.RealTime ? (
                <MapArea 
                  ...
                  onShowReportDetail={(report) => setSelectedReportDetail(report)}
                />
            ...
            {activeTab === NavigationTab.RealTime && (
              <aside 
                className={`transition-all duration-500 ease-in-out flex flex-col relative z-[2000] ${
                  isPanelOpen ? 'overflow-hidden' : 'overflow-visible'
                } ${
                  !isFullscreen 
                    ? 'rounded-2xl bg-white shadow-sm border border-slate-300 ml-2.5' 
